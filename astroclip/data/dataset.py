...
_URLS = {
    "spectrum": "/Users/marchuertascompany/Documents/data/EUCLID/Q1/spectra/euclid_Q1_test.hdf5",
}


class AstroClipEuclidSpectraDataset(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(
            name="spectrum",
            version=VERSION,
            description="This part of the dataset covers examples Euclid spectra",
        ),
    ]

    DEFAULT_CONFIG_NAME = "spectrum"

    def _info(self):
        if self.config.name == "spectrum":
            features = datasets.Features(
                {
                    #"image": datasets.Array3D(shape=(224, 224, 3), dtype="float32"),
                    "spectrum_flux": datasets.Array2D(shape=(531, 1), dtype="float32"),
                    "spetrum_error":datasets.Value("float32"),
                    "spectrum_mask":datasets.Value("int64"),
                    "spectrum_quality":datasets.Value("float32"),
        ...
        with h5py.File(filepath) as d:
            ...
            for j in range(dset_range[0], dset_range[1]):
                yield str(targetids[j]), {
                    ...
                    "spectrum_flux": np.reshape(spectra[j], [-1, 1]).astype("float32"),
